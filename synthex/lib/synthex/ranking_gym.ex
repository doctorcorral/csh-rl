defmodule Synthex.RankingGym do
  @moduledoc """
  ...
  defp solve_coorddescent(actions, opts) do
    ...
    {chain, _default} = Synthex.ChainGym.solve(chain_priority, default_action, chain_opts)
  ...
  defp solve_cegar(actions, opts) do
    ...
    {_final_part, _final_default, _final_anchors, best_part, best_default, best_successes} =
      Enum.reduce_while(1..cegar_rounds,
        ...
        # 1. Find overlap conflicts: states where multiple predicates
        #    fire with DIFFERENT rankings. Each is a CEX for whichever
        #    predicate disagrees with the oracle.
        ...
        # 2. Classify anchors by region (after overlap resolution)
        ...
        if total_cex == 0 and remaining_conflicts == 0 do
          ...
          # 3. Handle regular CEX — try regions in order of severity
          regions_by_severity =
            region_data
          ...
          {new_part, action_taken} =
            if n_overlaps_fixed > 0 do
              # Already made changes this round via overlap resolution
          ...
          # Always refresh anchors (even on no progress) to avoid stagnation
          new_anchors =
            if episodic? do
              current_anchors
            else
  ...
  # For each state where multiple predicates fire with different rankings,
  # the oracle tells us the true ranking.  Every predicate that disagrees
  # with the oracle at such a state is wrong and must be narrowed.
  defp resolve_overlaps(partition, anchors, oracle_rankings, action_names, env,
                        depth, max_coeff, top_k, n_verify) do
    # Collect (state, oracle_ranking, list of wrong predicate indices)
    conflicts =
      Enum.zip(anchors, oracle_rankings)
      |> Enum.flat_map(fn {state, oracle_r} ->
        ...
        if length(rankings) > 1 do
          # Multiple predicates fire with different rankings — find the wrong ones
          wrong_indices =
            matching
    ...
    if conflicts == [] do
      ...
      # Group wrong states by predicate index
      wrong_by_pred =
        conflicts
      ...
      # Narrow the most-affected predicate: exclude its overlap-wrong states
      ...
      # States in this predicate's region that ARE correct
      ok_in_region =
        Enum.zip(anchors, oracle_rankings)
  ...
  defp handle_default_cex(partition, default, worst_data, all_anchors, _all_oracle_rankings,
    ...
    # All distinct rankings observed in counterexamples
    ...
    # Collect pre-filtered predicates from all ranking groups (deduped)
    all_preds =
    ...
    if all_preds == [] do
      ...
      # Cross-product: each predicate × each observed ranking
  ...
  defp score_partition_candidates(partition, default, candidates, action_names, env, max_steps) do
    ...
    # Convert current partition to chain format: (pred, top_action_name)
    ...
    # Each candidate is {pred, ranking, f1} — convert to chain entry
    ...
    # Use the score command: chain_so_far + [(candidate, stage_action)]
    # We score each candidate as if inserting it at the end of the chain
    # To batch this, group by stage_action
