defmodule Synthex.Pipeline.TournamentImitation do
  ...
  alias Synthex.Permutation.TreeEngine
  alias Synthex.Permutation.TreePolicy
  ...
  def lunarlander_tournament_oracle([x, y, vx, vy, theta, _omega]) do
    ...
    best_action = case best_action do
  ...
  @doc """
  Returns a full ranking for the state based on the tournament oracle.
  Since the python script just returned the best action, we will build a ranking 
  where the best action is index 0, and the rest are arbitrary or sorted.
  """
  def lunarlander_oracle_ranking(state_list) do
    ...
    # Put best first, others naturally
    [best | List.delete(actions, best)]
  end

  @doc """
  Collects trajectories by following the Tournament Oracle.
  """
  def collect_dataset(env_mod, num_episodes, steps_per_episode) do
    ...
    Enum.map(unique_states, fn s ->
      ...
      # Un-scale the state list to floats for the python-like oracle
  ...
  defp rollout_oracle(state, env_mod, 0), do: [state]
  defp rollout_oracle(state, env_mod, n) do
    if env_mod.terminal?(state) do
      ...
      # We apply the action for 5 substeps, like the real environment
  ...
  def solve() do
    ...
    all_rankings = Synthex.Permutation.Engine.permutations(actions)

    IO.puts("Building RankTree to imitate Tournament...")
    tree = build_imitation_tree(dataset, pruned_cands, all_rankings, eval_fn, 5) # Depth 5 tree
    
    final_error = Enum.reduce(dataset, 0, fn {_s, sl, true_rank}, acc ->
      pred_rank = TreePolicy.evaluate(tree, sl, eval_fn)
      acc + Synthex.Permutation.OracleEngine.ranking_error(pred_rank, true_rank)
    end)
  ...
  # --- Tree building logic adapted for pure classification ---

  def best_leaf_ranking(dataset, all_rankings) do
    all_rankings
    |> Enum.min_by(fn ranking ->
      Enum.reduce(dataset, 0, fn {_s, _sl, true_rank}, acc ->
        acc + Synthex.Permutation.OracleEngine.ranking_error(ranking, true_rank)
      end)
  ...
  defp leaf_error(dataset, all_rankings) do
    ...
    Enum.reduce(dataset, 0, fn {_s, _sl, true_rank}, acc ->
      acc + Synthex.Permutation.OracleEngine.ranking_error(best_rank, true_rank)
    end)
  end
  ...
  defp split_dataset(dataset, pred, eval_fn) do
    Enum.split_with(dataset, fn {_s, sl, _true_rank} ->
      Synthex.PredProg.eval(pred, sl, eval_fn)
    end)
  end
end
