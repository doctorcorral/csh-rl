defmodule Synthex.Permutation.OracleEngine do
  @moduledoc """
  ...
  alias Synthex.Permutation.Policy
  ...
  defp simulate_action_penalty(state, action, env_mod, _horizon, policy) do
    s_prime = env_mod.step(state, action)
    short_horizon = 50
    env_mod.penalty(state) + Synthex.Permutation.Oracle.penalty_rollout(s_prime, policy, env_mod, &Synthex.ContinuousFeatures.eval_feature/2, short_horizon)
  end
  ...
  defp cegar_loop(env_mod, states, eval_fn, depth, max_coeff, k_swaps, max_iters, iter, best_pol \\ nil) do
    ...
    actions = env_mod.actions()
    base_rankings = Synthex.Permutation.Engine.permutations(actions)
    swaps = Synthex.Permutation.Engine.generate_swaps(length(actions))
    ...
    if iter >= max_iters do
      # End loop, either due to hitting max iters or we couldn't even fit the dataset perfectly.
      IO.puts("\n==================================================")
