defmodule Synthex.StateCEGAR do
  ...
  def is_consistent_on?(p, anchors, action_a, action_b, env_mod, eval_fn, known_pairs) do
    Enum.all?(anchors, fn s ->
      ...
      if ans == :tie do
        ...
        Synthex.PredProg.eval(p, env_mod.state_to_list(s), eval_fn) == ans
      end
    end)
  end
  ...
  def find_cex(p, action_a, action_b, env_mod, eval_fn, known_pairs) do
    Enum.find_value(env_mod.starts(), nil, fn s0 ->
      traj = Oracle.collect(s0, p, action_a, action_b, env_mod, eval_fn, 50, known_pairs) # 50 step trajectory check
      Enum.find(traj, fn s ->
        ans = Oracle.oracle_predict(p, s, action_a, action_b, env_mod, eval_fn, known_pairs)
        ans != :tie and Synthex.PredProg.eval(p, env_mod.state_to_list(s), eval_fn) != ans
      end)
    end)
  end
