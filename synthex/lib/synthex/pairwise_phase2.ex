defmodule Synthex.PairwisePhase2 do
  @moduledoc """
  ...
  alias Synthex.{GymOracle, CEGIS, PredProg}
  ...
  def synthesize(chain, default_action, opts) do
    ...
    # For each region × pair, run CEGIS with progressive depth
    pair_predicates =
      for region_id <- Enum.to_list(0..(n_regions - 1)) ++ [-1] do
        label = if region_id == -1, do: "Default", else: "R#{region_id}"
          ...
          for {a, b} = pair <- all_pairs do
            ...
            if length(obs) < 3 do
              ...
              if ratio > 0.95 or ratio < 0.05 do
                ...
                # Progressive CEGIS: start at depth 0, go deeper if exhausted
  ...
  # Build depth-1 from only discriminative features to keep the space tractable.
  # A feature is discriminative if it is true for some positive and false for
  # some negative examples (or vice versa) — constant features can't help.
  defp try_depth_1(features, observations, eval_feat_fn) do
    ...
    if length(discriminative) == 0 do
      ...
      # Build depth-1 from discriminative atoms only
  ...
  defp build_pairwise_matrix(pair_results, _all_pairs, _n_actions,
                              ...
                              eval_feat_fn) do
    # Get representative states for this region
    region_states =
      all_states
      |> Enum.filter(fn s ->
        cond do
    ...
    for {pair, status, value, _ratio} <- pair_results, into: %{} do
      ...
      case status do
        ...
        :synthesized ->
          pred = value
          # Evaluate predicate on representative states: majority vote
        ...
        _ ->
          # Insufficient or exhausted — use Phase 1 ordering
          {{a, b}, false}
      end
    end
  end
  ...
  defp format_pred(pred, env) do
    Synthex.RankingGym.format_pred(pred, env)
  rescue
    _ -> inspect(pred)
  end
