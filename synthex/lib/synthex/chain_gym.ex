defmodule Synthex.ChainGym do
    ...
    {_final_chain, final_features, {best_chain, best_val, best_land}, _cex} =
      Enum.reduce(1..cegar_rounds, {initial_chain, features, initial_best, []}, fn cegar_iter, {chain, feats, best, cex_data} ->
        ...
        if cegar_iter < cegar_rounds and length(active) > 0 do
          ...
          # Generic: refine features from current policy's trajectories
          ...
          # Environment-specific: if explore is supported, also run StateCEGAR
          # to get per-state CEX data for hard consistency filtering
  ...
  defp optimize_position(action, default, chain_before, chain_after, features, max_depth, top_k, seeds, env, cex_data, max_steps) do
    ...
    # If CEX data exists, pre-filter by hard consistency (principled StateCEGAR).
    # No weights — viable_on? is a hard constraint, then we rank by episode reward.
    ...
    if max_depth == 0 do
      ...
      # Hard filter depth-1 candidates too
