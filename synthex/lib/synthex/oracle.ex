defmodule Synthex.Oracle do
  ...
  def tournament_action(state, p, action_a, action_b, known_pairs, env_mod, eval_fn) do
    # Use fallback action or the first action
  ...
  defp check_beats(state, a, b, p, action_a, action_b, known_pairs, env_mod, eval_fn) do
    cond do
      {a, b} == {action_a, action_b} -> Synthex.PredProg.eval(p, env_mod.state_to_list(state), eval_fn)
      {b, a} == {action_a, action_b} -> not Synthex.PredProg.eval(p, env_mod.state_to_list(state), eval_fn)
      Map.has_key?(known_pairs, {a, b}) and known_pairs[{a, b}] != :failed -> 
        Synthex.PredProg.eval(known_pairs[{a, b}], env_mod.state_to_list(state), eval_fn)
      Map.has_key?(known_pairs, {b, a}) and known_pairs[{b, a}] != :failed -> 
        not Synthex.PredProg.eval(known_pairs[{b, a}], env_mod.state_to_list(state), eval_fn)
      true -> false # Fallback: neither beats the other explicitly
    end
  end
