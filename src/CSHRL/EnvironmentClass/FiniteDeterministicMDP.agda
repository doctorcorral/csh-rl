...
module FiniteDeterministicMDP
  ...
  -- The ranking relation derived from finder
  _ranks_≤_ : State → Action → Action → Bool
  ...
  -- Extract head comparison from trace comparison
  trace-≤ₜ-head : ∀ r₁ t₁ r₂ t₂ → 
                  (r₁ ∷ t₁) ≤ₜ (r₂ ∷ t₂) ≡ true →
                  r₁ ≤ᵣ r₂
  trace-≤ₜ-head r₁ t₁ r₂ t₂ p with r₁ ≤? r₂ in eq
  ... | true  = ≤?-sound r₁ r₂ eq
  ... | false = case-false p
    where
      -- In the false case, the trace comparison evaluates to false,
      -- but p says it's true - contradiction
      case-false : false ≡ true → r₁ ≤ᵣ r₂
      case-false ()
