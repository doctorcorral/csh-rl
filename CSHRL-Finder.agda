...
-- Greater-than check for sorting (Descending order)
_>ₜ_ : Trace → Trace → Bool
t1 >ₜ t2 = if t1 ≤ₜ t2 then false else true
-- Wait, if t1 == t2, <= is true, so > is false. Correct.
-- If t1 < t2, <= is true, so > is false. Correct.
-- If t1 > t2, <= is false, so > is true. Correct.
...
-- 3. The Policy Finder (Single Best Action)
...
-- 4. The Ranking Finder (Full Permutation)
