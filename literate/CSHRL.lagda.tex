...
\node[box,fill=nord7!25,below left=1.5cm and 0.5cm of core] (fdmdp) {FiniteDeterministic\\MDP};
\node[box,fill=nord7!25,below right=1.5cm and 0.5cm of core] (cpmdp) {Combinatorial\\PlacementMDP};
...
\node[box,fill=nord12!25,right=0.8cm of queens] (maze) {Maze\\(classic)};
...
\subsection{Stochastic Environments via the Giry Monad}

Our current formalization handles deterministic environments. Extending to stochastic MDPs requires lifting stream dominance through probability distributions. We conjecture that the \emph{Giry monad}~[10] provides the correct categorical framework:

\begin{itemize}
\item Replace $\texttt{State} \to \texttt{Action} \to \texttt{State} \times \texttt{Reward}$ with $\texttt{State} \to \texttt{Action} \to \mathcal{G}(\texttt{State} \times \texttt{Reward})$, where $\mathcal{G}$ is the Giry monad on measurable spaces.
\item Stream dominance $\leq_s$ lifts to stochastic dominance over distributions of streams.
\item The coinductive homomorphism property becomes: rankings preserve \emph{expected} stream dominance in the monadic sense.
\end{itemize}
