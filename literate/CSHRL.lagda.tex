...
We prove four main results, all machine-verified in Agda:
...
\textbf{Stochastic extension:} This subsumption result lifts to stochastic MDPs. By linearity of expectation, comparing partial sums of expected rewards is equivalent to comparing expected partial sums---exactly what classical RL optimizes. Section~\ref{sec:stochastic} presents the stochastic framework; the Appendix provides the complete proof (\texttt{appendix/StochasticSubsumption.agda}).
...
We defined stream dominance coinductively: $x \le_s y$ iff heads compare and tails recursively compare. But is this the ``right'' definition? Does it capture exactly what we intuitively want---dominance at every timestep?

\textbf{Answer: Yes.} The coinductive definition is \emph{isomorphic} to pointwise dominance:
...
\begin{itemize}
\item \textbf{The definition is right:} The coinductive formulation exactly captures ``dominance at every timestep''---no more, no less. This isn't an approximation or abstraction; it's a precise characterization.
\item \textbf{Finder completeness:} The Finder compares traces at finite depths. The isomorphism shows that if traces dominate at all depths, infinite streams dominate coinductively. Finite-depth comparison is exact (at sufficient depth), not an approximation.
\item \textbf{Justifies ``Symmetric'':} The correspondence is bidirectional. Rankings that preserve stream order and stream orders that induce rankings are interchangeable views---hence ``symmetric'' homomorphism.
\end{itemize}

\noindent The full proof is in \texttt{appendix/StreamIsomorphism.agda} and the Appendix document.
...
\hline
\textbf{Giry Monad}~[10] & Probability distributions as a monad on measurable spaces & Direct application: stochastic CSHRL uses discrete Giry monad (\S\ref{sec:stochastic}) \\
\hline
\end{tabularx}
\end{table}
...
Skalse et al.~[12] introduce both value-based and policy-gradient algorithms that provably converge to lexicographically optimal policies. Their framework treats objectives as separate reward functions ordered by importance.
Tercan \& Prabhu~[13] address limitations of earlier heuristics---particularly cases where Bellman equations fail to respect lexicographic constraints---via the \emph{Lexicographic Projection Algorithm}.
...
\subsection{Preference-Based and Ranking RL}

Preference-based RL~[15] learns from pairwise comparisons rather than scalar rewards. This connects to CSHRL's ranking structure, but the relationship is nuanced:
\begin{itemize}
\item Preference-based RL \emph{infers a latent scalar} from comparisons, then optimizes that scalar.
\item CSHRL requires only \emph{ordinal structure}---a partial order on outcomes. The source is irrelevant: it could be environment rewards, human preferences, or any other ordinal signal.
\item The key difference: CSHRL \emph{compares streams directly} without collapsing to a scalar. Preference-based RL reconstructs a scalar and discards the comparison structure.
\end{itemize}

\noindent In fact, CSHRL's Core module is parameterized by \texttt{\_≤ᵣ\_ : Reward → Reward → Set}---any partial order suffices. Human preference orderings are a valid instantiation. The ``reward'' in CSHRL is not a number to be summed but an \emph{ordinal signal} to be compared.

\subsection{Coalgebraic Foundations}

The coinductive stream order $\le_s$ is a coalgebraic relation in the sense of Rutten~[5]. Coalgebra provides the categorical semantics for infinite behaviors, and our formalization directly instantiates these ideas in Agda's guarded coinduction. The \texttt{coinductive} keyword in the $\le_s$ record is precisely the coalgebraic final coalgebra construction.

MDP homomorphisms~[6] exploit state/action symmetries for computational efficiency. CSHRL uses a different notion of ``symmetry'': not group-theoretic automorphisms of the state space, but \emph{order-preservation} between rankings and stream dominance.
...
\node[groupbox,below=0.8cm of fdmdp] (fdtasks) {
  \begin{tikzpicture}
    ...
    \node[smallbox,right=0.1cm of t3] (t4) {DelayedGrat};
  \end{tikzpicture}
};
...
% Labels
\node[below=0.05cm of fdtasks,font=\tiny] {+demos (verified)};
\node[below=0.05cm of cptasks,font=\tiny] {verified};
\node[below=0.05cm of stasks,font=\tiny] {postulate-free (lex)};
\end{tikzpicture}
\caption{Environment class hierarchy with verified tasks. Core provides theory, ECs provide infrastructure, task groups contain domain-specific proofs. Deterministic (left): 6+ tasks. Stochastic (right): 4 tasks using lexicographic comparison.}
\end{figure}
...
\item \textbf{Required from task:} Bridge lemma or direct preservation proof
\end{itemize}

\noindent The full implementation is in \texttt{src/CSHRL/EnvironmentClass/FiniteDeterministicMDP.agda}. Most tasks inherit directly from this EC.

\subsection{CombinatorialPlacementMDP: Constraint Satisfaction}

A \emph{specialized} environment class for placement problems (N-Queens, Sudoku, Graph Coloring), extending FiniteDeterministicMDP with domain-specific structure. The full implementation is in \texttt{src/CSHRL/EnvironmentClass/CombinatorialPlacementMDP.agda}:
...
The deterministic framework extends naturally to probabilistic environments. We implement this via a \textbf{finite distribution monad}---a discrete approximation of the Giry monad~[10].
...
\noindent The full implementation is in \texttt{src/CSHRL/Probability/Finite.agda}.
...
\noindent The full implementation is in \texttt{src/CSHRL/Core/Stochastic.agda}, with the stochastic Environment Class in \texttt{src/CSHRL/EnvironmentClass/StochasticFiniteMDP.agda}.
...
\noindent Since pointwise implies lexicographic, any MDP satisfying our lexicographic preservation \emph{and} pointwise dominance (which includes all four verified examples) also subsumes classical argmax. The Appendix provides the complete proof in \texttt{appendix/StochasticSubsumption.agda}.
...
\draw[arr] (update) -- ++(0,-1.8) -| (sample);
\draw[arr] (test) -- node[right,font=\footnotesize,color=nord14] {ok} (refine);
\draw[arr] (refine) -| (sample);

\node[below=3.3cm of test,font=\footnotesize,color=nord3] {Loop until convergence};
\end{tikzpicture}
...
\noindent These features are demonstrated in \texttt{src/CSHRL/Tasks/Verified/CurriedLearnerDemo.agda}, which uses a ``Late Bloomer'' MDP where traces are indistinguishable at low depth but diverge later---validating that depth increase and violation-driven swaps correctly discover the optimal ranking.
...
\noindent All monotonicity guarantees carry over: swaps still strictly decrease violation count, convergence still holds. The full stochastic learning module is in \texttt{src/CSHRL/Learning/StochasticFiniteMDP.agda}.
...
\end{code}
\noindent The key insight: at depth 1, both actions yield trace $[0]$---indistinguishable. At depth 2, \texttt{GoA} yields $[0, 0]$ while \texttt{GoB} yields $[0, 1]$. Lexicographic comparison reveals the winner. The full implementation with \texttt{find-ranking} tests is in \texttt{src/CSHRL/Tasks/Classic/DelayedGratification.agda}.
...
\end{axis}
\end{tikzpicture}
\caption{Violations at Start state drop to zero as depth increases. Delayed Gratification requires depth 2 to distinguish equal-immediate-reward paths; Trap Sparse resolves at depth 1. Both are verified in \texttt{src/CSHRL/Tasks/Classic/}.}
\end{figure}
...
\subsection{Instant Adaptation to Action Unavailability}

Because CSHRL maintains a \emph{full ranking} over actions (not just a single best-action pointer), adaptation to unavailable actions is $O(1)$:

\begin{description}
\item[Robotics:] A manipulator arm has 6 joints. If joint 3 jams, the agent simply demotes all actions using joint 3 to ``unavailable'' and the next-best action is already ranked. \textbf{No replanning, no value recomputation.} Classical methods must re-solve the MDP with the reduced action space.

\item[Network Routing:] A router maintains rankings over outbound paths. When link A fails, packets immediately flow to the second-ranked path. \textbf{Failover is $O(1)$}, not $O(|V| \cdot |E|)$ as in shortest-path recomputation.

\item[Game AI:] An opponent blocks your preferred move. The backup is already ranked---no minimax tree search required for the next choice.
\end{description}

\subsection{Beyond Exploration vs.\ Exploitation}
...
\textbf{The key insight:} To correctly rank \emph{any} pair of actions---even the bottom two---the agent must understand both of their infinite futures. This means:

\begin{itemize}
\item Knowing that action $a_5$ is worse than $a_4$ requires understanding $a_5$'s full trajectory
\item By transitivity, correctly ordering the worst actions requires understanding the \emph{entire} action space
\item There is no ``exploitation without exploration''---the ranking \emph{is} the understanding
\end{itemize}
...
\subsection{Eliminating Common Failure Modes}

CSHRL's structure avoids several pathologies of scalar-based RL:

\begin{description}
\item[No bootstrapping instability:] Classical temporal difference methods chase moving value targets. CSHRL compares \emph{rankings}, which are discrete and stable. A swap either fixes a violation or it doesn't.

\item[No off-policy issues:] Every experience directly tests whether the current ranking agrees with the environment's stream dominance. There is no importance sampling, no replay ratio tuning.

\item[Forced long-term reasoning:] If two actions differ only at step 1000, lexicographic trace comparison \emph{must} discover this difference at sufficient depth. Discounted methods might wash out the distinction; CSHRL cannot.
\end{description}

\subsection{Provable Correctness}
...
\textbf{The Conjecture:} Define coinductive FOSD (comparing marginal distributions at each step) and pointwise FOSD (comparing at all timesteps). The Appendix conjectures these are isomorphic---extending the Stream Isomorphism (\S6.1) to distributions.
...
\subsection{Quantum Speedup for Violation Detection}

We conjecture that CSHRL's permutation structure admits quantum advantage. The intuition:

\begin{itemize}
\item Action rankings can be encoded as superpositions over the symmetric group $S_{|A|}$.
\item Violation detection becomes an oracle query: ``does this ranking have a violation at state $s$?''
\item Grover's amplitude amplification~[7,8] can detect violations in $O(\sqrt{|S_{|A|}|})$ queries instead of $O(|S_{|A|}|)$ classically.
\end{itemize}
...
\subsection{Additional Environment Classes}
...
The stochastic extension via the Giry monad shows that this principle lifts to probabilistic environments. The key insight: \textbf{lexicographic coinductive comparison} is the correct notion for expected value ordering. Tail comparison is conditional on head equality, which:
\begin{itemize}
\item Preserves the infinite tower structure (still coinductive)
\item Matches lexicographic trace semantics (earlier rewards break ties)
\item Makes impossible cases trivially satisfied (absurd pattern matching)
\end{itemize}

\noindent This resolves the apparent gap between trace and stream dominance, yielding a fully verified stochastic CoinFlip example with no postulates.
...
\bibitem{ref10}
Giry, M. (1982). A categorical approach to probability theory. In Categorical Aspects of Topology and Analysis, Lecture Notes in Mathematics, vol 915, pp.~68--85. Springer.

\bibitem{ref11}
Diaconis, P. (1988). Group representations in probability and statistics. IMS.

\bibitem{ref12}
Skalse, J., et al. (2022). Lexicographic Multi-Objective Reinforcement Learning. In Proceedings of the 31st International Joint Conference on Artificial Intelligence (IJCAI), pp.~3430--3436.

\bibitem{ref13}
Tercan, H., \& Prabhu, S. (2024). Thresholded Lexicographic Ordered Multiobjective Reinforcement Learning. In European Conference on Artificial Intelligence (ECAI).

\bibitem{ref14}
Garc{\'\i}a, J., \& Fern{\'a}ndez, F. (2015). A Comprehensive Survey on Safe Reinforcement Learning. Journal of Machine Learning Research, 16(1), 1437--1480.

\bibitem{ref15}
Wirth, C., et al. (2017). A Survey of Preference-Based Reinforcement Learning Methods. Journal of Machine Learning Research, 18(136), 1--46.
