...
\subsection{Exploration and Exploitation as Group Operations}

The permutation structure provides natural exploration-exploitation trade-offs:

\begin{description}
\item[Exploitation:] Select the top-ranked action. Trivial.

\item[Exploration:] Sample uniformly from the symmetric group $S_{|A|}$~[11]. Each permutation represents a hypothesis about the true ranking. Violation detection tests these hypotheses directly.

\item[Epsilon-greedy analog:] With probability $\epsilon$, apply a random transposition to the ranking before acting. This explores ``nearby'' policies in permutation space.
\end{description}
