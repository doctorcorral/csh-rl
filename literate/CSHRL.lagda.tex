...
We also present a \textbf{stochastic extension} via the Giry monad, lifting the coinductive homomorphism to probabilistic environments where rankings preserve \emph{expected} stream dominance.
...
\item \textbf{Deterministic and stochastic step functions:} Deterministic ECs use \texttt{step : State → Action → State × Reward}; stochastic ECs (\S\ref{sec:stochastic}) use \texttt{step : State → Action → Dist (State × Reward)} with finite discrete distributions.
...
\begin{tikzpicture}[
    ...
    box/.style={draw,rounded corners,minimum height=2.5em,minimum width=7em,align=center,font=\small},
    inherit/.style={->,thick,>=Stealth,dashed},
    arr/.style={->,thick,>=Stealth}
]
\node[box,fill=nord9!20] (core) {Core.agda\\(Theory)};
\node[box,fill=nord15!20,right=2cm of core] (stoch) {Core/Stochastic\\(Giry monad)};
\node[box,fill=nord7!25,below left=1.5cm and 1cm of core] (fdmdp) {FiniteDeterministic\\MDP};
\node[box,fill=nord7!25,below=1.5cm of core] (cpmdp) {Combinatorial\\PlacementMDP};
\node[box,fill=nord15!30,below right=1.5cm and 1cm of core] (smdp) {Stochastic\\FiniteMDP};
\node[box,fill=nord13!30,below=1.5cm of fdmdp] (two) {TwoState\\(verified)};
\node[box,fill=nord13!30,below=1.5cm of cpmdp] (queens) {Queens1\\(verified)};
\node[box,fill=nord14!40,below=1.5cm of smdp] (coin) {CoinFlip\\(verified)};
...
\node[below=0.3cm of coin,font=\footnotesize] {No postulates (lex)};
\end{tikzpicture}
\caption{Environment class hierarchy: Core provides theory, ECs provide verified infrastructure, tasks provide domain-specific proofs. The stochastic branch uses expected stream dominance.}
\end{figure}
...
The key insight for stochastic MDPs: \textbf{pointwise stream dominance is too strong}. Instead, we use \textbf{lexicographic coinductive comparison}---the tail comparison is only required when expected heads are equal:
...
\begin{itemize}
\item \textbf{Pointwise} $\Rightarrow$ \textbf{Lexicographic}: If $x \le_s y$ pointwise at every step, then certainly $x \le_{lex} y$.
\item \textbf{Lexicographic} $\not\Rightarrow$ \textbf{Pointwise}: Lexicographic is strictly weaker.
\item \textbf{Deterministic case}: Both coincide, since a strict head inequality determines the entire trajectory.
\end{itemize}

\noindent This explains why the deterministic theory ``just works'' while stochastic requires the refined notion.

\subsection{Stochastic Learning}

The learning infrastructure extends directly. The only change: violation detection uses \emph{expected traces} instead of deterministic traces.

\begin{code}[hide]
  open import Data.Maybe using (Maybe; just; nothing)
  open import Data.Bool using (Bool; true; false)
  
  module StochasticLearningTypes 
    (State Action Reward : Set)
    (expected-trace-action : State → Action → ℕ → List Reward)
    (_≤ₜᵇ_ : List Reward → List Reward → Bool)
    (finder-ranking : ℕ → State → Action → Action → Bool)  -- Internal, derived from Dec
    where
...
\begin{code}
    -- Violation test: compare expected traces
    -- finder-ranking is derived from Dec internally (not a parameter)
    ...
    ... | true  | false = just (violation s b a k)  -- Ranking disagrees with traces
    ... | _     | _     = nothing                    -- No violation
\end{code}

\noindent The full stochastic learning module is in \texttt{src/CSHRL/Learning/StochasticFiniteMDP.agda}.
...
\textbf{Stochastic learning:} The same infrastructure extends to stochastic MDPs (\texttt{CSHRL.Learning.StochasticFiniteMDP}). The only change: violation detection compares \emph{expected} traces instead of deterministic traces. All monotonicity guarantees carry over.
...
\subsection{Stochastic Environments: Current Status}

We have implemented a discrete stochastic extension via the Giry monad (\S\ref{sec:stochastic}). The current status:

\textbf{Implemented and Verified:}
\begin{itemize}
\item Finite distribution monad (\texttt{CSHRL.Probability.Finite})
\item Stochastic step functions returning \texttt{Dist (State × Reward)}
\item \textbf{Lexicographic coinductive comparison} (\texttt{\_≤ₛ-lex\_})---the correct notion for stochastic MDPs
\item Stochastic Environment Class (\texttt{StochasticFiniteMDP})
\item Stochastic learning module with expected trace comparison
\item \textbf{Fully verified CoinFlip example} (no postulates, \texttt{--safe})
\end{itemize}

\textbf{Key theoretical result:} The apparent gap between trace dominance and stream dominance is resolved by using \emph{lexicographic} coinductive comparison, where tail comparison is conditional on head equality. This preserves the infinite tower structure while correctly capturing the semantics of expected value comparison.

\textbf{Open directions:}
\begin{itemize}
\item \textbf{Continuous distributions:} Extend from \texttt{List (A × ℕ)} to measure-theoretic distributions.
\item \textbf{Risk-aware objectives:} Beyond expected value---CVaR, worst-case, distributional dominance.
\item \textbf{More stochastic examples:} Gambler's Ruin, Multi-armed Bandits, etc.
\end{itemize}
