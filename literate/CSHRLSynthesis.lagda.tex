...
\textbf{(13)}~\emph{Compositional Ranking Algebra}: an algebraic framework for composing independently-verified rankings---proving that \texttt{prefix-sum} is a \emph{linear operator} (distributes over $+$ and $\times$), that \texttt{sd-weight} inherits this linearity, and that FOSD and SD$[k]$ are closed under distribution concatenation (\texttt{SD-++}) and reward scaling (\texttt{SD-scale}); a \texttt{VerifiedRanking} abstraction with \textbf{product composition} (two independently-verified components yield a verified product), \textbf{sum composition} (disjoint environments compose with dispatched rankings), and \textbf{scaling} (reward amplification preserves verification)---enabling modular, compositional verification where each component is verified in isolation and composed automatically.
...
\item \textbf{Compositional Ranking Algebra (\S\ref{sec:compose}):} An algebraic framework for composing independently-verified rankings, implemented in \texttt{Probability.Compose} and \texttt{Core.Compose}.  Key results:
\begin{itemize}
...
\item \textsc{Product Composition}: if two independent components have \texttt{VerifiedRanking}s at SD$[k]$, the product (coordinated) ranking is verified at SD$[k]$ for the composite.
...
\item Compose the proofs via \texttt{product-ranking}, \texttt{sum-ranking}, or \texttt{scale-ranking}.
\end{enumerate}
The composed proof is as strong as the component proofs---no additional verification is needed at the composite level.  This mirrors how software verification handles large systems: verify modules independently, then compose via interfaces.
\end{remark}
