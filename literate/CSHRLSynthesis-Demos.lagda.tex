...
\subsection{Demo 5: 8-Queens (Scaling)}
...
\subsection{Demo 6: N-Queens with Feature Discovery (CPMDP)}
\label{sec:e2e-auto}
...
\medskip
{\footnotesize \textcolor{nord0}{\LARGE\sffamily ♛}~=~given prefix \qquad \textcolor{nord10}{\LARGE\sffamily ♛}~=~auto-completed by synthesized program}
...
\medskip
{\footnotesize \textcolor{nord11}{\bfseries$\times$}~=~conflict / Dead \qquad \textcolor{nord0}{\LARGE\sffamily ♛}~=~given \qquad \textcolor{nord10}{\LARGE\sffamily ♛}~=~auto-completed}
...
\subsection{Demo 7: 1D Maze with Feature Discovery (FDMDP)}
\label{sec:e2e-maze-auto}

The FTL for placement problems (Demo~6) derives features from \emph{state structure}: pairwise comparisons on \texttt{List~ℕ}.  Finite-state MDPs have \emph{atomic} states---no internal structure to compare.  But features can be derived from the \emph{dynamics}: the step function gives states their meaning.
...
\begin{enumerate}
\item \textbf{Structural FTL} (Demo~6): pairwise comparisons on indexed data (\texttt{List~ℕ} states), for placement and assignment problems.
\item \textbf{Dynamics FTL} (Demos~7--8): dynamics-derived properties (reward, self-loops, terminal reachability) for finite-state MDPs.
\end{enumerate}
...
\subsection{Demo 8: FrozenLake \texorpdfstring{$4 \times 4$}{4x4} (FDMDP)}
...
\subsection{Demo 9: Cliff Walking \texorpdfstring{$4 \times 6$}{4x6} (FDMDP)}
...
\subsection{Demo 10: Auto-FTL --- Zero Human Feature Engineering}
\label{sec:e2e-autoftl}

Demos~8 and~9 demonstrated fully automated policy synthesis, but both relied on \emph{human-defined} feature templates: \texttt{row-is}, \texttt{col-is}, and dynamics features specific to the grid interpretation.  A natural question is: can the features themselves be generated automatically?
...
The cascading CEGIS pipeline from Demo~8 works \emph{unchanged} with the auto-generated features:
...
\paragraph{Learning curve dynamics in FrozenLake $8{\times}8$.}
To further validate the sample efficiency and cross-pair propagation in a larger setting, we evaluate the rollout performance of partial policies during the synthesis loop on the 64-state FrozenLake environment (53 non-terminal states). Measuring performance as the number of starting states from which the agent successfully reaches the goal, the formally verified learning curve exhibits a distinct step-like structure:
\begin{center}
\small
\begin{tabular}{@{}rlcc@{}}
\textbf{Queries} & \textbf{Policy behavior} & \textbf{Success} & \textbf{Success (\%)} \\
\hline
...
\end{tabular}
\end{center}
For the first 35 observations (which happen to be optimally served by the default action or are safely navigable), the CEGIS synthesizer requires no complex predicates, so the policy remains effectively ``Always go Down,'' yielding a baseline success rate of 7/53. The breakthrough occurs precisely at \textbf{observation 36} (corresponding to grid state 38). At state 38, going Down leads directly into a hole (state 46), so the oracle forces a \texttt{Right} action. CEGIS is then forced to wake up and discover a predicate that isolates state 38. Crucially, it does not just memorize this state; it \emph{propagates} its structural insight (e.g., using factorization features) to other unobserved regions sharing the same geometry. This instantly jumps the real-world performance to 15/53. Finally, processing the last few critical edge cases refines the version space to the exact optimal policy, achieving 100\% success (53/53). This confirms that CEGIS performance leaps discontinuously exactly when it encounters structurally informative observations.

\paragraph{Stochastic learning curve: Slippery FrozenLake $4{\times}4$.}
We replicate this experiment in the stochastic setting using the slippery FrozenLake environment. Here, we track \emph{policy accuracy} (how many of the 11 non-terminal states the synthesized policy classifies identically to the memoized depth-10 expected-value oracle) as observations accumulate. The curve reveals a similar step-wise structural generalization:
\begin{center}
\small
\begin{tabular}{@{}rlcc@{}}
\textbf{Queries} & \textbf{Policy behavior} & \textbf{Accuracy} & \textbf{Accuracy (\%)} \\
\hline
...
\centering
\begin{tikzpicture}[
    cell/.style={draw=nord3, minimum width=2.5cm, minimum height=1.2cm, align=center, font=\footnotesize},
    obs/.style={fill=nord10!20, draw=nord10, very thick},
    dissol/.style={fill=nord9!10},
    cross/.style={fill=nord14!20},
    both/.style={fill=nord13!20},
    arr/.style={->, thick, nord10, shorten >=2pt, shorten <=2pt},
    carr/.style={->, thick, nord14, shorten >=2pt, shorten <=2pt, dashed}
]

% Column Headers
\node[font=\bfseries\small] at (0, 1.2) {A vs B};
\node[font=\bfseries\small] at (3, 1.2) {\dots};
\node[font=\bfseries\small] at (6, 1.2) {C vs D};

% Row Headers
\node[anchor=east,font=\small] at (-1.5, 0) {$s_1$ (Class $\alpha$)};
\node[anchor=east,font=\small] at (-1.5, -1.6) {$s_2$ (Class $\alpha$)};
\node[anchor=east,font=\small] at (-1.5, -3.2) {$s_3$ (Class $\beta$)};

% Grid Cells
\node[cell, cross] (ab1) at (0,0) {Deduced\\$A > B$};
\node[cell, draw=none] (dots1) at (3,0) {\dots};
\node[cell, obs] (cd1) at (6,0) {\textbf{Observed}\\$C > D$};

\node[cell, both] (ab2) at (0,-1.6) {Deduced\\$A > B$};
\node[cell, draw=none] (dots2) at (3,-1.6) {\dots};
\node[cell, dissol] (cd2) at (6,-1.6) {Subsumed\\$C > D$};

\node[cell] (ab3) at (0,-3.2) {?};
\node[cell, draw=none] (dots3) at (3,-3.2) {\dots};
\node[cell] (cd3) at (6,-3.2) {?};

% Arrows
\draw[arr] (cd1.south) -- node[right,font=\scriptsize,align=left] {Dissolution\\($s_2 \sim s_1$)} (cd2.north);
\draw[carr] (cd1.west) -- node[above,font=\scriptsize] {Cross-Pair Prop.} (ab1.east);

% Diagonal implication (both)
\draw[->, thick, nord13, dotted, shorten >=2pt, shorten <=2pt] (cd1.south) to[bend left=20] (ab2.east);

\end{tikzpicture}
\caption{The Multiplier Effect in Version Space. A single observation for a sub-optimal pair (e.g., $C \text{ vs } D$ at $s_1$) simultaneously resolves multiple dimensions of the policy. Vertical dissolution (solid blue arrow) propagates the observation to all states in the same feature equivalence class ($\alpha$). Horizontal cross-pair propagation (dashed green arrow) generalizes the learned rule to all action pairs sharing the same feature structure. The combination yields a multiplicative $O(k^2 \cdot m)$ reduction in sample complexity.}
\label{fig:cross-pair-matrix}
\end{figure}
...
\subsection{Demo 14: FrozenLake \texorpdfstring{$4 \times 4$}{4x4} (Slippery) --- Stochastic Synthesis}
\label{sec:e2e-frozenlake-slippery}

Demos~8--13 operated in the \emph{deterministic} setting: actions have a single, predictable outcome.
...
The grid and map topology are identical to Demo~8 (same 16~states, same 4~holes and 1~goal), but each chosen action has only $\tfrac{1}{3}$ probability of executing in the intended direction, with $\tfrac{1}{3}$ each for the two perpendicular directions (e.g., choosing Right yields \{Right, Down, Up\}, each with weight~1 out of~3).
...
\end{enumerate}
The pipeline scales to the 8×8 grid (Demo~15, \S\ref{sec:e2e-frozenlake-8x8-slippery}).
...
\subsection{Demo 15: FrozenLake \texorpdfstring{$8 \times 8$}{8x8} (Slippery) --- Scaling the Stochastic Pipeline}
...
\begin{center}
\begin{tabular}{@{}lcc@{}}
& \textbf{Demo 14 (4×4)} & \textbf{Demo 15 (8×8)} \\
\hline
