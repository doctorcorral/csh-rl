...
continuous actions---across \textbf{thirteen environments}:
Pendulum, LunarLander, CartPole, MountainCar, Acrobot, Pong,
Breakout, BipedalWalker, Swimmer (MuJoCo), Hopper (MuJoCo),
CliffWalking, Slippery CliffWalking, and FrozenLake 8$\times$8 Slippery.
...
\item \textbf{Binary-weighted decomposition} (\S\ref{sec:bipedal}):
      ...
      spaces (BipedalWalker, Swimmer, Hopper).
\end{enumerate}
...
This section details the two oracle-based synthesis methods:
\textbf{flat-partition CEGAR} for direct full-ranking synthesis
(Pendulum) and \textbf{coordinate descent with full-ranking
determination} for episodic environments (LunarLander).
Partition-based search (\S\ref{sec:partition}) and
binary-weighted decomposition (\S\ref{sec:bipedal}) are
described in their respective sections.  All four methods
produce full-ranking partitions using simulation-in-the-loop
evaluation.
...
episodes---episode reward directly captures landing
...
The inverted pendulum has 3-dimensional continuous state
$(\cos\theta, \sin\theta, \omega)$ and 3 discrete actions:
\texttt{TorqueNeg} ($-2$), \texttt{NoTorque} ($0$),
\texttt{TorquePos} ($+2$).
...
Table~\ref{tab:all-results} collects all thirteen environments,
...
Hopper-v5
  & Binary decomp. & 3$\times$8 & 11 & 9 & 1098.2/ep (97\%) & --- \\
\midrule
CliffWalking
...
\bottomrule
\end{tabular}
\caption{All thirteen environments with synthesis method, predicate
...
\item \textbf{Mixed complexity}: some bits use single atomic
  features (e.g., knee2's $w{=}1$ bit is $-2 \dot\theta_2 +
  \omega_1 < 0$), while others require conjunctions or
  disjunctions of two atoms.  No bit exceeds depth~1.
  The synthesis naturally discovers the minimal predicate
  complexity needed for each bit's contribution to the gait.
\end{itemize}
...
\paragraph{Setup.}
The 8D observation vector is:
$[\alpha_0,\, \alpha_1,\, v_x,\, v_y,\,
  \dot\theta_0,\, \dot\theta_1,\, \dot\theta_2,\, \dot\theta_3]$
(two joint angles, 2D center-of-mass velocity, four angular
velocities).  With $b = 3$ bits per dimension and $d = 2$
action dimensions, the decomposition yields 6~binary predicates
(vs.\ 12 for BipedalWalker).  Each predicate decides one bit of
one joint torque.  The same coordinate-descent + CEGAR pipeline
is used, with 3077~features generated from trajectory states.
Synthesis completed in approximately 17~hours (3~CEGAR rounds,
5~iterations each).
...
BipedalWalker (Box2D, 24D state, 12~predicates),
Swimmer (MuJoCo, 8D state, 6~predicates), and
Hopper (MuJoCo, 11D state, 9~predicates) are solved by
...
\subsection{Hopper-v5 (MuJoCo)}

Hopper is a 3D single-legged robot that must learn to hop forward.
The environment has an 11-dimensional continuous state
(torso height, angle, three joint angles, and their velocities) and
a 3-dimensional continuous action space (thigh, leg, and foot torques)
in $[-1, +1]$.  Hopper is substantially harder than Swimmer: it
requires dynamic balance during ballistic flight phases, and naive
policies collapse immediately.

\paragraph{Why Hopper matters.}
Hopper tests whether predicate synthesis can discover a
\emph{dynamic locomotion gait}---not just steady-state actuation
(Swimmer) or quasi-static walking (BipedalWalker), but a cyclical
pattern of launching, flying, and landing on a single leg.
The physics are unforgiving: incorrect torque timing leads to
immediate falls.

\paragraph{Setup.}
The observation vector is mapped as:
$z = \text{obs}[0]$ (torso height),
$\theta = \text{obs}[1]$ (angle),
$\theta_{\text{thigh}} = \text{obs}[2]$,
$\theta_{\text{leg}} = \text{obs}[3]$,
$\theta_{\text{foot}} = \text{obs}[4]$,
$v_x = \text{obs}[5]$,
$v_z = \text{obs}[6]$,
$\omega = \text{obs}[7]$ (angular velocity),
$\dot\theta_{\text{thigh}} = \text{obs}[8]$,
$\dot\theta_{\text{leg}} = \text{obs}[9]$,
$\dot\theta_{\text{foot}} = \text{obs}[10]$.
Three bits per action dimension give $2^3 = 8$ torque levels per joint,
9~predicates total.  The default Gymnasium healthy reward ($+1$ per
timestep for remaining upright) was set to zero, forcing the synthesis
to optimize purely for forward velocity.

\paragraph{Synthesized predicates.}
The final policy (3~CEGAR rounds, 5~coordinate-descent iterations each):

\begin{center}
\small
\begin{tabular}{@{}llll@{}}
\textbf{Joint} & \textbf{Bit} & \textbf{Weight} & \textbf{Predicate} \\
\hline
Thigh & 0 & 1 & $5\theta_{\text{thigh}} + \theta_{\text{foot}} < 0 \;\wedge\; -\dot\theta_{\text{foot}}^2 + \omega < 0$ \\
Thigh & 1 & 2 & $\dot\theta_{\text{thigh}} < -1.33 \;\vee\; z \cdot \theta < -0.01$ \\
Thigh & 2 & 4 & $-v_z + \omega < 0 \;\wedge\; \theta < 0.15$ \\
\hline
Leg & 0 & 1 & $-5\theta^2 + \omega < 0 \;\vee\; -0.01 v_x^2 + \omega < 0$ \\
Leg & 1 & 2 & $-0.1 v_x^2 + \omega < 0 \;\wedge\; 5\theta^2 + \theta_{\text{thigh}} < 0$ \\
Leg & 2 & 4 & $\theta_{\text{thigh}} \dot\theta_{\text{thigh}} < 0.5 \;\wedge\; \theta_{\text{thigh}} \omega < 0.35$ \\
\hline
Foot & 0 & 1 & $\theta_{\text{foot}} \dot\theta_{\text{foot}} < -2.06 \;\vee\; v_x < 0.76$ \\
Foot & 1 & 2 & $-0.5 z^2 + v_x < 0 \;\vee\; \theta_{\text{foot}} v_x < 0.07$ \\
Foot & 2 & 4 & $4\dot\theta_{\text{leg}}^2 + v_z < 0 \;\vee\; 5\dot\theta_{\text{leg}}^2 + \omega < 0$ \\
\end{tabular}
\end{center}

\paragraph{Deployment results.}
Final validation: \textbf{avg 1098.2/ep}, \textbf{193/200 episodes survived}
(1000-step horizon, healthy\_reward${}=0$).
Across spot-check seeds, episode rewards range from 773 to 1338.
The policy discovers a rhythmic hopping gait driven primarily by
angular velocity ($\omega$) and joint-angle phasing.

\begin{remark}[Stability--aggressiveness trade-off]
With healthy\_reward${}=0$, the synthesis optimizes purely for
forward velocity, producing an aggressive hopping style.
Some episodes terminate early due to falls after vigorous jumps.
Restoring a positive healthy reward would shift the trade-off
toward stability at the cost of reduced forward speed---a tunable
parameter, not a limitation of the framework.
\end{remark}

\begin{remark}[Comparison with neural baselines]
PPO baselines for Hopper-v5 typically achieve 2000--3000/ep with
$\sim$100K parameters.  Our 9-predicate policy reaches $\sim$1100/ep
with zero neural parameters.  The gap reflects the expressiveness
ceiling of 3-bit torque resolution (8~levels per joint), not a
failure of predicate discovery.  Increasing to 4~bits (16~levels)
or adding a healthy reward bonus would likely close the gap
substantially.
\end{remark}
...
The most striking result is the compression ratio.  All thirteen
...
$2\theta + \dot\theta < 0$.  The LunarLander policy is three
...
A human can read $2\theta + \dot\theta < 0 \Rightarrow \text{Left}$
...
physics variables).  BipedalWalker, Swimmer, and Hopper (\S\ref{sec:bipedal})
...
near-solved performance (296.8/ep, 351.6/ep, 1098.2/ep).  The remaining genuine
...
\item \textbf{Compactness as a scientific finding.}  All thirteen
...
\item \textbf{Continuous actions via binary decomposition.}
      ...
      survival.  Hopper-v5 (MuJoCo) is controlled by 9~predicates
      (3~bits $\times$ 3~joints), achieving 1098.2/ep with 97\%
