...
\institute{Independent Researcher, Chihuahua, Mexico\\
\email{ricardo@gyx.ai}}
...
All definitions, theorems, and algorithms presented in this paper have been formalized and machine-checked in Agda 2.8.0 (Standard Library 2.0).
...
Concrete module paths are referenced throughout the text; the full artifact is publicly available at \url{https://github.com/doctorcorral/csh-rl}.
...
\noindent In words: if the ranking says~$b$ is at least as good as~$a$, then the state reached by~$b$ must have a value stream that dominates the value stream of the state reached by~$a$.
...
The map $a \mapsto \mathit{value}(\mathit{next}(s,a))$ sends each action to a stream, and the definition requires this map to be \emph{order-preserving} from the action ranking into the stream ordering.
...
We chose Agda over other proof assistants (Coq, Isabelle, Lean) because the development is coinduction-heavy: Agda's copatterns and \texttt{--guardedness} discipline let stream definitions and bisimulation proofs be written in direct style, without the coinduction encodings these systems typically require.  The entire development compiles under \texttt{--safe} (no postulates, no escape hatches), and literate Agda lets this paper itself be the checked artifact.  Nothing in the theory is Agda-specific; ports to other assistants are possible.
...
\section{Discussion: Strengths and Limitations}\label{sec:discussion}

\paragraph{Strengths.}
The framework eliminates the discount factor as an optimality requirement, replacing it with pointwise stream dominance, and it correctly ranks sacrifice patterns---verified in \texttt{BinarySacrifice}, \texttt{SkillInvestment}, and \texttt{PreparationDilemma} (\S\ref{sec:strict-generality})---where discounted return provably fails (\S\ref{sec:subsumes}).  Every result is machine-checked in Agda under \texttt{--safe} with zero postulates, so there are no unstated assumptions.  The full-ranking stance yields concrete operational benefits: convergent learning without a separate exploration mechanism (Remark~\ref{rem:exploration}) and $O(1)$ adaptation to action unavailability (Remark~\ref{rem:adaptation}).

\paragraph{The ideal versus the machinery.}
It is important to distinguish the \emph{coinductive ideal} from the \emph{algorithmic machinery}.  The definitions in \S\ref{sec:agda-core} are about infinite streams and pointwise dominance at every time step; the \texttt{CoinductiveHomomorphism} condition is undecidable in general.  We view this as analogous to AIXI \cite{Hutter2005}: an uncomputable ideal that organizes a hierarchy of practical approximations.  The Finder (\S\ref{sec:finder}) compares \emph{finite} traces at a fixed depth and becomes exact under conditions verified by the Environment Class infrastructure (e.g., horizon sufficiency); the Learner (\S\ref{sec:learning}) wraps Finder-style comparisons into a learning loop.  Crucially, the full condition \emph{is} achievable for structured domains: the 8-Queens instance attains it by exploiting binary reward structure and finite game depth.

\paragraph{Limitations.}
The deterministic framework assumes a finite action space, a deterministic step function, decidable reward order, and a horizon parameter for the Finder approximation (Section~\ref{sec:stochastic} lifts to stochastic environments; Section~\ref{sec:abstraction} to infinite state spaces).  Scaling to \emph{continuous} action spaces remains open (\S\ref{sec:future}).  Because computing exact traces requires access to the step function, the current concrete instantiations sit closer to model-based RL and formal planning than to pure model-free RL; the learning loop itself, however, only consumes pairwise comparisons and is agnostic to how the oracle obtains them.  Finally, this paper's contribution is foundational and its validation is by machine-checked instantiation rather than empirical benchmark; an empirical evaluation of the heuristic, full-ranking synthesis this theory licenses---in robotics and safety-critical control, where verified fallback rankings under actuator failure are directly valuable---is ongoing work beyond the scope of this paper.
