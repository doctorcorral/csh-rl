...
\paragraph{Strengths.}
The framework eliminates the discount factor as an optimality requirement, replacing it with pointwise stream dominance, and it correctly ranks sacrifice patterns where discounted return provably fails (\S\ref{sec:subsumes}).  Every result is machine-checked in Agda under \texttt{--safe} with zero postulates, so there are no unstated assumptions.  The full-ranking stance yields concrete operational benefits: convergent learning without a separate exploration mechanism (Remark~\ref{rem:exploration}) and $O(1)$ adaptation to action unavailability (Remark~\ref{rem:adaptation}).
...
\item \textbf{Build:} Compiling the literate source with \texttt{agda --latex} type-checks every embedded definition and emits the \LaTeX{} rendered here; \texttt{lualatex} then produces the PDF. The full verified development is available at \url{https://github.com/doctorcorral/csh-rl}.
