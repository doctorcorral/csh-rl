...
In many practical settings---robotics, network routing, game playing---actions can become unavailable at runtime (e.g., a joint fails, a resource is exhausted, or an opponent blocks a move).  Classical RL typically stores only the argmax action.  When that action becomes unavailable, the policy must be recomputed, usually at cost~$O(|S| \cdot |A|)$ or worse.

Modern neural networks can produce multiple ranked actions quickly (e.g., via top-$k$ logits).  However, while the top action is usually reliable, the relative ordering among the remaining actions is typically not trustworthy: the network was trained to identify the single best action, not to produce a well-calibrated total ranking.  Secondary and tertiary preferences are often poorly justified.

Our framework maintains a \emph{total ranking} over all actions at every state, together with a machine-checked proof that the ranking satisfies the homomorphism property.  Because preservation is a pairwise property, the relative order between any two actions remains valid even after some actions are removed.  Consequently, when the top-ranked action becomes unavailable, the second-ranked action is already known to be the best among the remaining ones---an $O(1)$ lookup with a formal guarantee.

\begin{theorem}[Restricted Preservation]
If a ranking $R$ satisfies the homomorphism property for action set $\mathcal{A}$, and $\mathcal{A}' \subseteq \mathcal{A}$ is the set of currently available actions, then the restriction $R|_{\mathcal{A}'}$ satisfies the same property for $\mathcal{A}'$.
\end{theorem}

\noindent The core step---demoting an unavailable action leaves the relative order of every remaining pair unchanged---is machine-checked in \fpath{src/CSHRL/Learning/Base.agda} (\texttt{demote-preserves-dominance}), and applies to both \texttt{CoinductiveHomomorphism} and \texttt{CoindHomo} without retraining or runtime re-verification.
