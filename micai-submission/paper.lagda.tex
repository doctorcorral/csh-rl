...
This is where the section title is literal: a swap of two actions is a \emph{transposition}, and transpositions generate the symmetric group~$S_{|\mathcal{A}|}$ (\S\ref{sec:formal}).  Learning is therefore a walk through~$S_{|\mathcal{A}|}$, applying transpositions until the ranking reaches the permutation that the environment's stream order dictates---restoring the correct symmetric structure one violated pair at a time.
